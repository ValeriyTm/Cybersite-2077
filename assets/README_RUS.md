...
│   │   │   │   ├── support/         # Логака работы технической поддержки
│   │   │   │   ├── trading/         # Логика работы с избранным/корзиной
...
[![Смотреть демонстрацию проекта](./PreviewYoutube.png)](https://www.youtube.com/watch?v=7y4Q71LkbKo)
...
![The whole project demo](./Project.gif)
...
- К CI подключен OWASP Dependency-Check, который сканирует все зависимости проекта (включая транзитивные) на наличие известных уязвимостей.
...
- Оплата заказа производит изменение его статуса с PENDING на PAID, а также происходит конечное списание остатков (quantity уменьшается, reserved обнуляется) на складах.
  -Для безопасности при получении ответа от ЮKassa реализован ответный запрос на официальный ip с пришедшим id, чтобы убедиться в валидности данных перед тем, как будет осуществляться бизнес-логика приложения.
  -Изменение статуса оплаты заказа сопровождается генерацией событий с последующим оповещением в ТГ.
...
4. Из корня проекта выполните сборку фронтенда:

```bash
npm run build -w web
```

5. Запуск контейнеров. Выполните из корня проекта:
...
6. Необходимо настроить ngrok, иначе в приложении не будет работать функционал оплаты.
...
7. После всех действий приложение будет запущено на http://localhost. БД уже будет заполнена и вы сможете зайти в приложение под аккаунтом дефолтного админа (login - admin@cybersite2077.com, password - AdminPassword2077!).

8. Для удобной работы с базами данных также запукаются контейнеры с Adminer (для PostgreSQL), Mongo-Express (для MongoDB) и Redis-Insight (для Redis).
