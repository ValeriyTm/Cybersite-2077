describe("Проверка фунцкионала аутентификации и редактирования профиля", () => {
  ...
  it("Должны отправиться инструкции по сбросу пароля на почту", () => {
  ...
  //Тест №10: Проверяем вход в аккаунт с 2FA:
  it("Проверяем вход в аккаунт с 2FA", () => {
    //1)Перехват запросов (чтобы не использовать реальные данные):
    //Перехватываем запросы при попытке залогиниться:
    cy.intercept("POST", "**/identity/auth/login", {
      statusCode: 200,
      body: {
        requires2FA: true,
        userId: "12345",
    ...
    //Перехватываем запросы при 2FA:
    cy.intercept("POST", "**/identity/auth/2fa/verify", {
      statusCode: 200,
      body: {
        accessToken: "fake-token",
        user: { name: "AdminUser", email: "admin@test.com" },
      },
    }).as("verify2FA");

    //2) Логинимся как обычно
    cy.visit("/auth");
    ...
    //3)Ждем появления формы ввода кода 2FA:
    cy.wait("@loginRequires2FA");

    //4)Вводим код подтверждения:
    cy.get('input[name="code"]').type("123456");
    cy.get('input[name="code"]').type("{enter}");
    //5)Уведомление об успешном входе:
    cy.contains(/вход выполнен успешно/i).should("be.visible");
  });
});
